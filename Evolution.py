...
class Evolution:
    ...
    def Generation(self):
        ...
        self.balls = BallNet.BallNet(self.number_of_balls - self.saved_balls, [self.starting_position[0],
                                     ...
                                     self.height, self.window, self.target, self.generation_num)
    ...
    def Mutation(self, child):
        ...
        for x in random.sample(range(self.moves), mutation_per):
            child[x] = [random.randrange(-1, 2), random.randrange(-1, 2)]
        return child
    ...
    def Step(self, frozen):
        if self.balls.dead_count < self.number_of_balls and self.current_moves < self.moves:
            self.balls.move(int(self.generation_num), self.current_moves, frozen)
